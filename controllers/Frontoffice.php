...
class Frontoffice extends CI_Controller {
	...
	//===========================================END TAMBAHAN KHUSUS VERIFIKASI============================================================
								   ...
								   xhr: function() {
									   ...
									   xhr.upload.addEventListener(\"progress\", function(evt){
										   if (evt.lengthComputable) {
											   ...
											   if(percentComplete==1){
												   loading.html('<h1>'+Math.round(percentComplete*100)+'% sending complete</h1><br> Tunggu beberapa saat lagi sampai proses penulisan di basisdata tujuan selesai sempurna...<br><i class=\"fa-3x fas fa-spinner fa-pulse\" ".$this->config->item('style_progres_bulat_admin')."></i>');
											   }else{
									  ...
									  xhr.addEventListener(\"progress\", function(evt){
										  if (evt.lengthComputable) {
											  ...
											  if(percentComplete==1){
												  loading.html('<h1>'+Math.round(percentComplete*100)+'% download complete</h1><br> Tunggu beberapa saat lagi sampai proses penulisan di basisdata tujuan selesai sempurna...<br><i class=\"fa-3x fas fa-spinner fa-pulse\" ".$this->config->item('style_progres_bulat_admin')."></i>');
											  }else{
		...
		//jadi bidang bisa meneruskan balik ke sekfretariat, tetapi inshaa Allah coba cek lagi lan.
			...
			if(isset($_POST['data_post_enkrip_hex'])){
					...
					foreach($data_post_terima as $key=>$k){
						if(!($key=='handle_hex_surat') && !($key=='handle_hex_berkas')){
							if($key=='timestamp_masuk'){
								...
								array_push($buffer,"UPT ".$this->config->item('nama_opd'));//sesuaikan jawaban ini dengan bidangnya, jika ini di sekretariat maka ganti dengan sekretariat '.$this->config->item('nama_opd').'
				...
				//redirect($_POST['asal_surat'].'/'.$hasil_insersi_surat_berkas);
					...
					echo "Surat gagal tercatat di UPT PPK BKD";
				}
...
/*
	...
	public function export2pdf_tinymce(){
		...
		export_html_ke_pdf($file_html,$output_dest='D',$_POST['nama_file'],$nama_satker='BKD Prov. Sulawesi Selatan',$nama_bidang='UPT BKD',$lebar_page=270,$tinggi_page=330,$orientasi='');
	}
	...
	public function terima_surat_masuk()
	{
		...
		*/
			if(isset($_POST['data_nama'])){
				...
				$directory_relatif_file_upload='./public/surat_dan_berkas_masuk/';	
				$upload=array();
				$upload1=upload('nama_file_surat', $folder=$directory_relatif_file_upload, $types="bbc,doc,bbc,doc,pdf,jpeg,gif,png,docs,docx,xls,xlsx,ppt,pptx,txt,sql,csv,xml,json,rar,zip,bmp,jpg,htm,html");
	...
	public function sekretariat_unggahsuratbaru()
	{
		...
		$coba[9][7]='UPT BKD';
		//$coba[10][0]='combo_manual';
		//$coba[10][7]=array('Sekretaris Badan','Kasubbag Program','Kasubbag Keuangan','Kasubbag Umum, Kepegawaian, Yang Lain (Others)');
		$coba[11][0]='hidden';
		$coba[11][7]=implode("-",array (date("d/m/Y"),mt_rand (1000,9999),microtime()));
	...
	public function tampilkan_tombol_baca_surat_obselet(){
		...
		echo "
			...
			<div class=\"card border-left-primary shadow h-100 py-2\">
				...
				<div class=\"row no-gutters align-items-center\">
					<div class=\"col mr-2\">
					<div class=\"text-xs font-weight-bold text-primary text-uppercase mb-1\">Kepala UPT PPK</div>
			...
			<div class=\"card border-left-success shadow h-100 py-2\">
				...
				<div class=\"row no-gutters align-items-center\">
					<div class=\"col mr-2\">
					<div class=\"text-xs font-weight-bold text-success text-uppercase mb-1\">Sub - </div>
					<div class=\"text-xs font-weight-bold text-success text-uppercase mb-1\">Nama - </div>
					<div class=\"text-xs font-weight-bold text-success text-uppercase mb-1\">Nip -</div>
					</div>
					<div class=\"col-auto\">
					<!--<i class=\"fas fa-dollar-sign fa-2x text-gray-300\"></i>-->
					</div>
	...
	public function frontoffice_index()
	{
		...
		*/
			if(isset($_POST['data_nama'])){
				...
				$upload=array();
				$upload1=upload('nama_file_surat', $folder=$directory_relatif_file_upload, $types="bbc,doc,bbc,doc,pdf,jpeg,gif,png,docs,docx,xls,xlsx,ppt,pptx,txt,sql,csv,xml,json,rar,zip,bmp,jpg,htm,html");
				$upload2=upload('nama_file_berkas', $folder=$directory_relatif_file_upload, $types="bbc,doc,bbc,doc,pdf,jpeg,gif,png,docs,docx,xls,xlsx,ppt,pptx,txt,sql,csv,xml,json,rar,zip,bmp,jpg,htm,html");
